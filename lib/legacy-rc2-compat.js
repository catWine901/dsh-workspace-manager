import { a as composeProfilePatches, d as readManagerLayerPatches, f as loadOptionalPatches, l as prepareManagerRuntimeLayer, n as ProfileRuntime, p as loadOverlayPatches, t as PROFILE_RUNTIME_SERVICE, u as profileRuntimeControl } from "./profile-runtime-bridge-BcnK0ROA.js";
...
//#region lib/types/legacy-rc2-compat.js
/**
...
const LEGACY_RC2_COMPAT_ENTRY_ID = "page-app-manager-legacy-rc2-compat";
...
function explicitlyDisablesCompat(layer) {
	return applyEntryPatches([{
		id: LEGACY_RC2_COMPAT_ENTRY_ID,
		name: `${MANAGER_PACKAGE}/legacy-rc2-compat`
	}], structuredClone([...layer]), () => {}).find((row) => row.id === LEGACY_RC2_COMPAT_ENTRY_ID)?.disabled === true;
}
...
function orderedAnchorPatchIndexes(layer) {
	return layer.flatMap((patch, patchIndex) => {
		...
		return rows.some((row, index) => {
			...
			return first.id === "page-app-manager-legacy-rc2-compat" && first.name === `${MANAGER_PACKAGE}/legacy-rc2-compat` && second.id === MANAGER_ENTRY_ID && second.name === MANAGER_PACKAGE;
		}) ? [patchIndex] : [];
	});
}
...
async function disposeLegacyRc2FiberAfterReadyFailure(ctx, error, activeTimeoutMs = 1e4) {
	...
	if (ctx.fiber.state === 0 || ctx.fiber.state === 1) try {
...
function awaitLegacyRc2FiberActive(ctx, timeoutMs = 1e4) {
	const stateError = () => {
		...
		if (ctx.fiber.state === 3 || ctx.fiber.state === 4 || ctx.fiber.state === 5) return /* @__PURE__ */ new Error(`${NAME}: compatibility fiber exited before becoming active`);
	};
	const initialError = stateError();
	if (ctx.fiber.state === 2) return Promise.resolve();
	...
	return new Promise((resolveBarrier, rejectBarrier) => {
		...
		inspect();
		if (ctx.fiber.state !== 2 && !terminalFiberState(ctx.fiber.state)) watchdog = setTimeout(() => {
